...
def formatar_planilha(df):
    ...
    # Adicionar linha de somatório (exceto para Valor Débito TVI e Valor da NFe)
    colunas_para_somar = [col for col in colunas_presentes if col != 'Valor da NFe']
    soma = df[colunas_para_somar].sum(numeric_only=True)

    linha_soma = {col: soma[col] if col in soma else '' for col in df.columns}
    df = pd.concat([df, pd.DataFrame([linha_soma])], ignore_index=True)

    # Calcular multa e total com multa para "ICMS Débito"
    multa = soma.get('ICMS Débito', 0) / 2
    total_com_multa = soma.get('ICMS Débito', 0) + multa
    ...
    df = pd.concat([df, pd.DataFrame([linha_multa, linha_total_multa])], ignore_index=True)

    # Reordenar colunas: colocar BC + 50%, Aliq Interna e ICMS Débito depois de Valor da NFe
